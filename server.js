...
function createVideoEl(peerId, labelText) {
  ...
  video.autoplay = true;
  video.playsInline = true; // important on mobile
  video.muted = false;
...
function createPeerConnection(peerId) {
  ...
  // Add local tracks immediately (before offers)
  if (localStream) {
...
async function handleSignal({ from, signal }) {
  ...
  if (signal.type === 'offer') {
    ...
    // Some implementations send candidates in the same channel
    try {
...
socket.on('participants', (ids) => {
  ...
  ids.filter(id => id !== socket.id).forEach(id => {
    // Create placeholder card early for UX
    createVideoEl(id, `Peer ${id.slice(0,4)}`);
    makeOffer(id);
  });
});
